...
NTask<N, lambda, types...>::NTask(lambda &func, types &...args)
    : Step<decltype(math ::Get), res_tuple_type>(math ::Get, res),
      tasks(detail ::make_n_tuple<N - 1, Task<lambda, types...>, lambda,
                                  types...>(func, args...)) {}
...
constexpr decltype(auto) NTask<N, lambda, types...>::get_impl() noexcept {
  if constexpr (n < N - 1)
    return std ::tuple_cat(std ::make_tuple(std ::get<n>(this->tasks)()),
                           this->get_impl<n + 1>());
  else

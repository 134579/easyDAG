...
Step<lambda, types...>::Step(lambda &func, types &...args)
    : func(std ::forward<lambda>(func)), args(std ::tie(args...)),
      _name("Task") {}
...
constexpr auto Step<lambda, types...>::operator+(T x) noexcept {
  return Step<decltype(math ::Add_lambda), Step<lambda, types...>, T>(
      math ::Add_lambda, *this, x);
}

template <typename lambda, typename... types>
template <class T>
constexpr auto Step<lambda, types...>::operator-(T x) noexcept {
  return Step<decltype(math ::Sub_lambda), Step<lambda, types...>, T>(
      math ::Sub_lambda, *this, x);
}

template <typename lambda, typename... types>
template <class T>
constexpr auto Step<lambda, types...>::operator*(T x) noexcept {
  return Step<decltype(math ::Mul_lambda), Step<lambda, types...>, T>(
      math ::Mul_lambda, *this, x);
}

template <typename lambda, typename... types>
template <class T>
constexpr auto Step<lambda, types...>::operator/(T x) noexcept {
  return Step<decltype(math ::Div_lambda), Step<lambda, types...>, T>(
      math ::Div_lambda, *this, x);
}

template <typename lambda, typename... types>
template <class T>
constexpr auto Step<lambda, types...>::operator==(T x) noexcept {
  return Step<decltype(math ::Eq_lambda), Step<lambda, types...>, T>(
      math ::Eq_lambda, *this, x);
}

template <typename lambda, typename... types>
template <class T>
constexpr auto Step<lambda, types...>::operator!=(T x) noexcept {
  return Step<decltype(math ::NEq_lambda), Step<lambda, types...>, T>(
      math ::NEq_lambda, *this, x);
}

template <typename lambda, typename... types>
template <class T>
constexpr auto Step<lambda, types...>::operator<(T x) noexcept {
  return Step<decltype(math ::Lower_lambda), Step<lambda, types...>, T>(
      math ::Lower_lambda, *this, x);
}

template <typename lambda, typename... types>
template <class T>
constexpr auto Step<lambda, types...>::operator>(T x) noexcept {
  return Step<decltype(math ::Greater_lambda), Step<lambda, types...>, T>(
      math ::Greater_lambda, *this, x);
}
...
std ::ostream &operator<<(std ::ostream &os, Step<lambda, types...> x) {
  ...
  if constexpr (utils ::has_symbol<lambda>::value)
    print<std ::tuple_size<decltype(kwargs)>::value - 1, lambda ::symbol()>(
        os, kwargs);
  else
...
template <typename lambda, typename... types>
template <class Func, std ::size_t idx, class... Args>
constexpr decltype(auto)
Step<lambda, types...>::traverse_impl(Func &func, Args &&...args) noexcept {
...
constexpr decltype(auto)
Step<lambda, types...>::traverse(Func &func, Args &&...args) noexcept {
...
constexpr void Step<lambda, types...>::dump_impl(std ::ostream &os) noexcept {
  ...
  {
    os << "  " << this->get_name() << " -> "
       << std ::get<idx>(this->args).get_name() << std ::endl;
...
constexpr void Step<lambda, types...>::dump(std ::ostream &os) noexcept {
  os << "  " << this->get_name()
     << "[shape=box, style=filled,color=\".7 .3 1.0\"]" << std ::endl;
  this->dump_impl<0>(os);
}

template <typename lambda, typename... types>
constexpr void
Step<lambda, types...>::graphviz(std ::ostream &os,
                                 const std ::string &graph_name) noexcept {
...
template <typename lambda, typename... types>
constexpr Step<lambda, types...> make_step(lambda &func,
                                           types... args) noexcept {

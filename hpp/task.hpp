...
Task<lambda, types...>::Task(lambda &func, types &...args)
    : Step<lambda, types...>(func, args...) {}
...
constexpr void Task<lambda, types...>::eval_impl() noexcept {
  ...
  // else if constexpr ( utils :: is_variable < idx_t > :: value ) // it is a
  // variable
  //   return;
}
...
constexpr void Task<lambda, types...>::eval() noexcept {
  this->eval_impl<0>();
  this->value = std ::async(std ::launch ::async,
                            &Step<lambda, types...>::operator(), this);
}
...
constexpr void Task<lambda, types...>::graph_impl(std ::ostream &os) noexcept {
  ...
  if constexpr (utils ::is_step<idx_t>::value) // it is a step
    ...
    os << std ::string(indent, ' ') << std ::get<idx>(this->args).get_name()
       << " [variable]" << std ::endl;
}
...
constexpr void Task<lambda, types...>::graph(std ::ostream &os) noexcept {
  ...
  os << std ::string(indent, ' ') << this->_name << " [" << cnt.num_variables
     << " # variables, " << cnt.num_operations << " # operations]"
     << std ::endl;

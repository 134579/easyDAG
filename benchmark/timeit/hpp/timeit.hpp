...
timeit<Num, Nrep>::timeit()
    : _min_time(std ::numeric_limits<double>::infinity()), _speedup(0.) {
  _times = std ::make_unique<double[]>(Nrep);
}

template <int Num, int Nrep> double timeit<Num, Nrep>::speedup() {
  return _speedup;
}
...
template <int Num, int Nrep> double timeit<Num, Nrep>::what_time_is_it_now() {
...
timeit<Num, Nrep>::what_time_is_it_now() {
